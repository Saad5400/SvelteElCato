<script>
    ...
    onMount(() => {
        let canvas = document.querySelector(".hacker-3d-shiz");
        if (!canvas) {
            return;
        }
        let ctx = canvas.getContext("2d");
        let canvasBars = document.querySelector(".bars-and-stuff");
        if (!canvasBars) {
            return;
        }
        let ctxBars = canvasBars.getContext("2d");
...
// Init graphics stuff
        ...
        function consoleOutput() {
            ...
            setTimeout(consoleOutput, ~~(Math.random() * 100));
        }
...
<div dir="ltr">
    ...
    <div class="output-console text-lime-400"></div>
</div>
