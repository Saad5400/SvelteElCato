...
export const load: PageLoad = async ({parent, params}) => {
    ...
    let question = quiz.questions.find(s => s.id === params.questionId) || error(404);
    const questionIndex = quiz.questions.indexOf(question);
    const next = quiz.questions.length > questionIndex + 1 ? quiz.questions[questionIndex + 1] : null;
